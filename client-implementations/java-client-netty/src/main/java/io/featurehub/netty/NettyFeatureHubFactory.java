...
public class NettyFeatureHubFactory implements FeatureHubClientFactory {
  @Override
  @NotNull
  public Supplier<EdgeService> createSSEEdge(@NotNull FeatureHubConfig config, @Nullable InternalFeatureRepository repository) {
    return () -> new NettySSEClient(repository, config, EdgeRetryer.EdgeRetryerBuilder.anEdgeRetrier().sse().build());
  }
  ...
  public Supplier<EdgeService> createRestEdge(@NotNull FeatureHubConfig config,
    ...
    return () -> new NettyRestClient(repository, config, EdgeRetryer.EdgeRetryerBuilder.anEdgeRetrier().rest().build(), timeoutInSeconds, amPollingDelegate);
  }
  ...
  public Supplier<TestApi> createTestApi(@NotNull FeatureHubConfig config) {
    return () -> new NettyTestClient(config);
  }
}
